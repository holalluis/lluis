<?php include'header.php'?>
<b>Sobre mí</b>
<p>
  Benvingut/da a la meva pàgina personal: aquí hi trobaràs articles sobre Linux,
  software lliure, matemàtiques, física, química i biologia.
</p>
<ul>
  <li>Treballo al centre de recerca <a href=//github.com/icra target=_blank>ICRA</a> (des del 2013).
  <li>Màster en Tecnologies de la Informació (<a href=//udg.edu target=_blank>UdG</a> 2012).
  <li>Llicenciat en Biotecnologia (<a href=//uab.cat target=_blank>UAB</a> 2011).
  <li>Sóc d'Anglès (Girona), però visc a Torroella de Montgrí (Baix Empordà).
  <li>Vaig néixer a Girona el 1989.
</ul>

<b>Links</b>
<ul>
  <li><a href="//github.com/holalluis">github</a>
  <li><a href="mail.php">mail</a>
  <li><a href="//www.linkedin.com/in/holalluis">linkedin</a>
  <li><a href="//www.twitter.com/holalluis">twitter</a>
</ul>
